...
const DynamicTourPage = () => {
  ...
  const [dynamicContent, setDynamicContent] = useState(null);
    ...
    .then(([itinData, destData]) => {
      let itin = null;
      if (itinData.success) {
        itin = itinData.data.find(d => d?.slug?.replace(/-$/, '') === slug?.replace(/-$/, ''));
      }

      let dest = null;
      if (destData.success) {
        dest = destData.data.find(d => d?.slug?.replace(/-$/, '') === slug?.replace(/-$/, ''));
      }
  ...
  const defaultHighlights = [
    " • Immersion dans les villages indiens",
    " • Rencontres humaines authentiques",
    " • Expériences rurales loin du tourisme classique",
    " • Cuisine locale et traditions familiales",
    " • Communauté Bishnoï & artisanat traditionnel",
    " • Rajasthan rural & désertique",
    " • Marchés colorés et vie quotidienne",
    " • Assistance francophone Indeora Voyages",
  ];
  const highlights = dynamicContent?.highlights !== undefined ? dynamicContent.highlights : defaultHighlights;
...
Amandine a été le début de notre super séjour in Inde et la clé indispensable à notre départ.
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      case "ITINÉRAIRE":
        return (
          <div className="pt-12 pb-16 px-6 md:px-12 lg:px-16 animate-fadeIn">
            <div className="mb-12">
              <h2 className="text-[#000] font-serif text-[28px] md:text-[32px] font-bold mb-6 italic leading-tight">{dynamicContent?.heroTitle || "Rajasthan Rural Inde Authentique — 13 Jours"} — {dynamicContent?.heroDuration ? (String(dynamicContent.heroDuration).toLowerCase().includes('jour') ? dynamicContent.heroDuration : `${dynamicContent.heroDuration} Jours`) : ""}</h2>
              <p className="text-[#000] font-['Montserrat'] leading-[1.8] text-[15px] max-w-3xl">{dynamicContent?.heroDescription || "Un voyage hors du temps au cœur de l’Inde rurale. Vivez au rythme des villages du Rajasthan, partagez le quotidien des communautés locales et découvrez une hospitalité authentique pour une immersion humaine profonde et mémorable."}</p>
            </div>
            <div className="space-y-4 mb-16">
              {(dynamicContent?.itinerary !== undefined ? dynamicContent.itinerary : [
                { day: "1", title: "Arrivée à Delhi," },
                { day: "2", title: "Delhi → Mandawa," },
                { day: "3", title: "Mandawa → Bikaner," },
                { day: "4", title: "Villages autour de Bikaner," },
                { day: "5", title: "Bikaner → Khimsar," },
                { day: "6", title: "Khimsar → Jodhpur," },
                { day: "7", title: "Villages Bishnoï & Vie Rurale," },
                { day: "8", title: "Jodhpur → Pushkar," },
                { day: "9", title: "Pushkar → Jaipur," },
                { day: "10", title: "Villages autour de Jaipur," },
                { day: "11", title: "Jaipur → Agra," },
                { day: "12", title: "Agra → Delhi," },
                { day: "13", title: "Départ," }
              ]).map((item, index) => (
                <div
                  key={index}
      ...
      case "EN DÉTAIL":
        const defaultItinerary = [
          {
            day: "01", title: "Arrivée à Delhi", desc: (
              <div className="space-y-2">
                ...
                <p>• Première découverte de Delhi.</p>
                <p>• Balade dans les jardins de Lodhi.</p>
                <p>• Connaught Place et marchés locaux.</p>
                <p>• Première immersion dans l’ambiance indienne.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Une première rencontre entre modernité et traditions.</p>
              </div>
            )
          },
          {
            day: "02", title: "Delhi → Mandawa", desc: (
              <div className="space-y-2">
                ...
                <p>• Découverte des paysages ruraux.</p>
                <p>• Arrêts dans les villages locaux.</p>
                <p>• Chai traditionnel sur la route.</p>
                <p>• Découverte des havelis peintes.</p>
                <p>• Vie locale du Rajasthan.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Architecture ancienne et traditions du Rajasthan.</p>
              </div>
            )
          },
          {
            day: "03", title: "Mandawa → Bikaner", desc: (
              <div className="space-y-2">
                ...
                <p>• Écoles rurales.</p>
                <p>• Troupeaux de chameaux.</p>
                <p>• Découverte de la vieille ville.</p>
                <p>• Marchés traditionnels.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Un Rajasthan désertique et authentique.</p>
              </div>
            )
          },
          {
            day: "04", title: "Villages autour de Bikaner", desc: (
              <div className="space-y-2">
                <p>• Rencontre avec des familles locales.</p>
                <p>• Cuisine traditionnelle avec les habitants.</p>
                <p>• Découverte des maisons en terre.</p>
                <p>• Vie quotidienne du désert.</p>
                <p>• Artisans et tisserands locaux.</p>
                <p>• Musique et traditions rurales.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Une immersion humaine loin du tourisme classique.</p>
              </div>
            )
          },
          {
            day: "05", title: "Bikaner → Khimsar", desc: (
              <div className="space-y-2">
                <p>• Route vers les villages du désert.</p>
                <p>• Safari en jeep dans les villages Bishnoï.</p>
                <p>• Rencontre avec les communautés rurales.</p>
                <p>• Vie nomade du désert.</p>
                <p>• Coucher du soleil dans les dunes.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Le désert du Rajasthan dans toute son authenticité.</p>
              </div>
            )
          },
          {
            day: "06", title: "Khimsar → Jodhpur", desc: (
              <div className="space-y-2">
                <p>• Découverte de la ville bleue.</p>
                <p>• Clock Tower Market.</p>
                <p>• Temples cachés.</p>
                <p>• Rues bleues et marchés.</p>
                <p>• Rencontre avec artisans locaux.</p>
                <p>• Fabrication textile et épices.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Entre désert et patrimoine royal.</p>
              </div>
            )
          },
          {
            day: "07", title: "Villages Bishnoï & Vie Rurale", desc: (
              <div className="space-y-2">
                <p>• Rencontre avec la communauté Bishnoï.</p>
                ...
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Une journée profondément authentique et humaine.</p>
              </div>
            )
          },
          {
            day: "08", title: "Jodhpur → Pushkar", desc: (
              <div className="space-y-2">
                ...
                <p>• Marchés colorés.</p>
                <p>• Rencontres spirituelles.</p>
                <p>• Cérémonie au bord du lac.</p>
          ...
          },
          {
            day: "09", title: "Pushkar → Jaipur", desc: (
              <div className="space-y-2">
                ...
                <p>• Quartiers anciens.</p>
                <p>• Atelier textile.</p>
                <p>• Cuisine locale et artisanat.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Ville royale et culture vivante.</p>
              </div>
            )
          },
          {
            day: "10", title: "Villages autour de Jaipur", desc: (
              <div className="space-y-2">
                <p>• Marchés de campagne.</p>
                <p>• Rencontre avec agriculteurs.</p>
                <p>• Vie quotidienne des villages.</p>
                <p>• Temples locaux.</p>
                <p>• Artisanat traditionnel.</p>
                <p>• Préparation du chai et cuisine familiale.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Une Inde rurale chaleureuse et authentique.</p>
              </div>
            )
          },
          {
            day: "11", title: "Jaipur → Agra", desc: (
              <div className="space-y-2">
                ...
                <p>• Traversée de villages ruraux.</p>
                <p>• Découverte du Taj Mahal au coucher du soleil.</p>
                <p>• Photographie architecturale moghole.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Romantisme et héritage moghol.</p>
              </div>
            )
          },
          {
            day: "12", title: "Agra → Delhi", desc: (
              <div className="space-y-2">
                <p>• Retour vers Delhi.</p>
                <p>• Temps libre pour les marchés artisanaux.</p>
                <p>• Derniers achats.</p>
                <p>• Détente et café.</p>
                <p>• Dîner d’adieu indien.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Une dernière immersion dans l’ambiance indienne.</p>
              </div>
            )
          },
          {
            day: "14", title: "Départ", desc: (
              <div className="space-y-2">
                ...
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Fin de votre immersion dans les villages de l’Inde.</p>
              </div>
            )
          },
        ];
        ...
        const activeItinerary = (formattedDynamicItinerary !== undefined) ? formattedDynamicItinerary : defaultItinerary;

        const selectedDay = activeItinerary[openDay] || activeItinerary[0] || { day: '', title: '', desc: '' };
        ...
        return (
          <div className="pt-12 pb-16 px-6 md:px-12 lg:px-16 animate-fadeIn">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {(dynamicContent?.accommodations !== undefined ? dynamicContent.accommodations : [
                {
        ...
        return (
          <div className="pt-12 pb-16 px-6 md:px-12 lg:px-16 animate-fadeIn">
            ...
            <div className="space-y-8">
              {(dynamicContent?.conseils !== undefined ? dynamicContent.conseils : [
                {
  ...
  return (
    <div className="w-full min-h-screen bg-[#f7f3f0] overflow-x-hidden">
      <SEO 
        ...
        customSlug={slug} 
        dynamicData={{ 
          title: dynamicContent?.heroTitle, 
          description: dynamicContent?.heroDescription || "Un voyage hors du temps au cœur de l’Inde.", 
          keywords: `${dynamicContent?.heroTitle}, voyage inde, indeora`, 
          image: dynamicContent?.heroImage || "https://indeoravoyages.com/wp-content/uploads/2025/08/rajan-768x611.jpg", 
          canonicalUrl: `https://indeoravoyages.com/destinations/${slug}` 
        }} 
      />
      ...
      <section className="relative h-[300px] md:h-[650px] lg:h-[720px] overflow-hidden">
        ...
        <div className="relative z-10 h-full flex flex-col items-center justify-center text-center w-full max-w-[1440px] mx-auto px-4 md:px-[40px] mt-8 md:mt-0">
          <div className="max-w-[800px]">
            <h1 className="font-serif text-[28px] md:text-[62px] lg:text-[76px] leading-[1.1] text-white mb-4 md:mb-8 drop-shadow-lg uppercase tracking-tight">{dynamicContent?.heroTitle || "Rajasthan Rural Inde Authentique 13 Jours"} — {dynamicContent?.heroDuration ? (String(dynamicContent.heroDuration).toLowerCase().includes('jour') ? dynamicContent.heroDuration : `${dynamicContent.heroDuration} Jours`) : ""}</h1>
            <div className="w-12 md:w-16 h-px bg-[#c58b32] mb-4 md:mb-6 mx-auto" />
            <p className="text-[11px] md:text-[16px] leading-relaxed text-white/90 max-w-[800px] mx-auto drop-shadow-md">{dynamicContent?.heroDescription || "Un voyage pensé pour les voyageurs en quête d’authenticité, de rencontres humaines et d’expériences locales loin des itinéraires classiques. Entre villages traditionnels, marchés ruraux, temples anciens et vie quotidienne, découvrez une Inde chaleureuse, vivante et profondément humaine."}</p>
          </div>
        </div>
      </section>
          ...
          <div className="mt-12 lg:mt-0 w-full lg:w-[380px] shrink-0 space-y-8">
            ...
            <div className="bg-white border border-[#eadfce]/40 rounded-sm p-10 shadow-[0_15px_45_rgba(70,45,20,0.12)]">
              ...
              <ul className="space-y-6">
                {(dynamicContent?.highlights !== undefined ? dynamicContent.highlights : highlights).map((point, index) => (
              ...
              {((dynamicContent?.highlights !== undefined ? dynamicContent.highlights : highlights).length > 3) && (
