      ...
      case "HÉBERGEMENT":
        return (
          <div className="pt-12 pb-16 px-6 md:px-12 lg:px-16 animate-fadeIn">
            ...
            <p className="text-gray-600 mb-10 leading-relaxed whitespace-pre-wrap">{dynamicContent?.accommodationText || "Nous avons sélectionné pour vous des établissements d'exception alliant charme architectural, confort moderne and hospitalité légendaire pour une immersion royale."}</p>
              ...
              <div>
                ...
                <ul className="space-y-2 text-sm text-gray-600 italic">
                  {(dynamicContent?.priceIncludes ? dynamicContent.priceIncludes.split('\n') : [
                      "• L'hébergement de 11 nuits en chambre double de catégorie charme",
                      "• Les petits-déjeuners gourmands",
                      "• Les transferts en véhicule privé climatisé avec chauffeur dévoué",
                      "• Toutes les visites, excursions et entrées mentionnées au programme (dont Taj Mahal, fort de Gwalior, temples de Khajuraho)",
                      "• La balade en bateau sur le Gange à Varanasi et la cérémonie spirituelle de l'Aarti"
              ...
              <div>
                ...
                <ul className="space-y-2 text-sm text-gray-600 italic">
                  {(dynamicContent?.priceExcludes ? dynamicContent.priceExcludes.split('\n') : [
                      ...
                      "• Les repas non mentionnés (déjeuners and dîners libres)",
                      "• Les pourboires d'usage pour le chauffeur and les guides"
                    ]).map((item, idx) => item.trim() ? <li key={idx}>{item}</li> : null)}
                </ul>
              </div>
            </div>
  ...
  return (
    <div className="w-full min-h-screen bg-[#f7f3f0] overflow-x-hidden">
      ...
      <section className="relative h-[300px] md:h-[650px] lg:h-[720px] overflow-hidden">
        <img src={dynamicContent?.heroImage || "/src/assets/image copy 21.png"} alt="La Vallée du Gange et Varanasi" className="absolute inset-0 w-full h-full object-cover" />
        ...
        <div className="relative z-10 h-full flex flex-col items-center justify-center text-center w-full max-w-[1440px] mx-auto px-4 md:px-[40px]">
          <div className="max-w-[800px] mt-8 md:mt-0">
            <h1 className="font-serif text-[24px] md:text-[62px] lg:text-[76px] leading-[1.1] text-white mb-2 md:mb-8 drop-shadow-lg uppercase tracking-tight">Delhi, Agra, Gwalior, Orchha, Khajuraho et Varanasi — 12 Jours</h1>
