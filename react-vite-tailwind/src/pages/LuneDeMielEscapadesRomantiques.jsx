...
const LuneDeMielEscapadesRomantiques = () => {
  ...
  const [dynamicContent, setDynamicContent] = useState(null);
    ...
    .then(([itinData, destData]) => {
      let itin = null;
      if (itinData.success) {
        itin = itinData.data.find(d => d.slug.replace(/-$/, '') === 'lune-de-miel-escapades-romantiques');
      }

      let dest = null;
      if (destData.success) {
        dest = destData.data.find(d => d.slug.replace(/-$/, '') === 'lune-de-miel-escapades-romantiques');
      }
  ...
  const defaultHighlights = [
    " • Palais romantiques du Rajasthan",
    " • Coucher du soleil dans les dunes",
    " • Danse Kalbeliya & dîner sous les étoiles",
    " • Taj Mahal au coucher du soleil",
    " • Plantations de thé du Kerala",
    " • Croisière privée en houseboat",
    " • Spa & détente ayurvédique",
      ...
      case "ITINÉRAIRE":
        return (
          <div className="pt-12 pb-16 px-6 md:px-12 lg:px-16 animate-fadeIn">
            <div className="mb-12">
              <h2 className="text-[#000] font-serif text-[28px] md:text-[32px] font-bold mb-6 italic leading-tight">{dynamicContent?.heroTitle || "Rajasthan Romantique Kerala — 14 Jours"} — {dynamicContent?.heroDuration ? (String(dynamicContent.heroDuration).toLowerCase().includes('jour') ? dynamicContent.heroDuration : `${dynamicContent.heroDuration} Jours`) : ""}</h2>
              <p className="text-[#000] font-['Montserrat'] leading-[1.8] text-[15px] max-w-3xl">{dynamicContent?.heroDescription || "Un voyage imaginé pour les couples en quête d’émotions, de moments suspendus et d’expériences inoubliables. Entre palais royaux, dunes du Rajasthan, backwaters du Kerala et couchers de soleil romantiques, vivez une lune de miel élégante et profondément romantique en Inde."}</p>
            </div>
            <div className="space-y-4 mb-16">
              {(dynamicContent?.itinerary?.length > 0 && typeof dynamicContent.itinerary[0]?.title === 'string' && dynamicContent.itinerary[0].title.trim() !== "" ? dynamicContent.itinerary : [
                ...
                { day: "2", title: "Delhi → Udaipur," },
                { day: "3", title: "Udaipur : Palais & Romance," },
                { day: "4", title: "Udaipur → Jodhpur," },
                { day: "5", title: "Jodhpur → Désert du Thar," },
                { day: "6", title: "Désert → Jaipur," },
                { day: "7", title: "Jaipur : Palais & Traditions," },
                { day: "8", title: "Jaipur → Agra," },
                { day: "9", title: "Agra → Kochi," },
                { day: "10", title: "Kochi → Munnar," },
                { day: "11", title: "Munnar : Nature & Détente," },
                { day: "12", title: "Munnar → Alleppey," },
                { day: "13", title: "Alleppey → Kochi," },
                { day: "14", title: "Départ" }
              ]).map((item, index) => (
                <div
                  key={index}
        ...
        const defaultItinerary = [
          {
            day: "01", title: "Arrivée à Delhi", desc: (
              <div className="space-y-2">
                ...
                <p>• Temps de repos.</p>
                <p>• Première balade à New Delhi.</p>
                <p>• Dîner romantique.</p>
                <p>• Ambiance intime et élégante.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Une arrivée douce avant le voyage romantique.</p>
              </div>
            )
          },
          {
            day: "02", title: "Delhi → Udaipur", desc: (
              <div className="space-y-2">
                <p>• Vol intérieur vers Udaipur.</p>
                <p>• Découverte du lac Pichola.</p>
                <p>• City Palace.</p>
                <p>• Vieille ville romantique.</p>
                <p>• Balade au coucher du soleil sur le lac.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">La ville la plus romantique du Rajasthan.</p>
              </div>
            )
          },
          {
            day: "03", title: "Udaipur : Palais & Romance", desc: (
              <div className="space-y-2">
                <p>• Découverte des palais royaux.</p>
                <p>• Jardins romantiques.</p>
                <p>• Temps libre en couple.</p>
                <p>• Rooftops élégants.</p>
                <p>• Dîner aux chandelles face au lac.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Une atmosphère élégante et intime.</p>
              </div>
            )
          },
          {
            day: "04", title: "Udaipur → Jodhpur", desc: (
              <div className="space-y-2">
                <p>• Route à travers les Aravalli.</p>
                <p>• Visite des temples jaïns de Ranakpur.</p>
                <p>• Villages ruraux.</p>
                <p>• Découverte de la ville bleue.</p>
                <p>• Rooftops romantiques.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Entre patrimoine et douceur du Rajasthan.</p>
              </div>
            )
          },
          {
            day: "05", title: "Jodhpur → Désert du Thar", desc: (
              <div className="space-y-2">
                <p>• Départ vers les dunes.</p>
                <p>• Installation dans le désert.</p>
                <p>• Balade à dos de chameau.</p>
                <p>• Coucher du soleil dans les dunes.</p>
                <p>• Danse Kalbeliya et musique traditionnelle.</p>
                <p>• Dîner sous les étoiles.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Une soirée magique et inoubliable.</p>
              </div>
            )
          },
          {
            day: "06", title: "Désert → Jaipur", desc: (
              <div className="space-y-2">
                <p>• Traversée des paysages désertiques.</p>
                <p>• Temps libre à Jaipur.</p>
                <p>• Découverte des bazars illuminés.</p>
                <p>• Moments de détente.</p>

                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Entre aventure et romantisme.</p>
              </div>
            )
          },
          {
            day: "07", title: "Jaipur : Palais & Traditions", desc: (
              <div className="space-y-2">
                <p>• Découverte du Fort d’Amber.</p>
                <p>• Hawa Mahal.</p>
                <p>• City Palace.</p>
                <p>• Atelier textile traditionnel.</p>
                <p>• Photographies en tenue traditionnelle.</p>
                <p>• Temps libre en couple.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Une immersion royale et élégante.</p>
              </div>
            )
          },
          {
            day: "08", title: "Jaipur → Agra", desc: (
              <div className="space-y-2">
                <p>• Arrêt à Chand Baori.</p>
                <p>• Traversée des villages du Rajasthan.</p>
                <p>• Découverte du Taj Mahal au coucher du soleil.</p>

                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Le symbole ultime du romantisme en Inde. </p>
              </div>
            )
          },
          {
            day: "09", title: "Agra → Kochi", desc: (
              <div className="space-y-2">
                <p>• Vol intérieur vers le Kerala.</p>
                <p>• Découverte de Fort Kochi.</p>
                <p>• Filets de pêche chinois.</p>
                <p>• Ambiance tropicale et paisible.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Transition vers le sud tropical de l’Inde. </p>
              </div>
            )
          },
          {
            day: "10", title: "Kochi → Munnar", desc: (
              <div className="space-y-2">
                <p>• Route vers les montagnes du Kerala.</p>
                <p>• Plantations de thé.</p>
                <p>• Cascades et vallées tropicales.</p>
                <p>• Temps libre pour spa et détente.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Nature et romantisme tropical.</p>
              </div>
            )
          },
          {
            day: "11", title: "Munnar : Nature & Détente", desc: (
              <div className="space-y-2">
                <p>• Balade dans les plantations.</p>
                <p>• Vue panoramique sur les montagnes.</p>
                <p>• Massage ayurvédique.</p>
                <p>• Temps libre en couple.</p>
                <p>• Détente dans la nature.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Calme et sérénité au cœur du Kerala.</p>
              </div>
            )
          },
          {
            day: "12", title: "Munnar → Alleppey", desc: (
              <div className="space-y-2">
                <p>• Route vers les backwaters.</p>
                <p>• Croisière privée en houseboat.</p>
                <p>• Coucher du soleil sur les canaux.</p>
                <p>• Dîner privé à bord.</p>
                <p>• Nuit paisible sur l’eau.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Une expérience romantique unique.</p>
              </div>
            )
          },
          {
            day: "13", title: "Alleppey → Kochi", desc: (
              <div className="space-y-2">
                <p>• Retour vers Kochi.</p>
                <p>• Shopping artisanal.</p>
                <p>• Café romantique.</p>
                <p>• Derniers moments de détente.</p>
                <p>• Dîner d’adieu.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Une dernière soirée romantique en Inde.</p>
              </div>
            )
          },
          {
            day: "14", title: "Départ", desc: (
              <div className="space-y-2">
                ...
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Fin de votre voyage romantique en Inde.</p>
              </div>
            )
          },
        ];
      ...
      <section className="relative w-full h-[320px] md:h-[650px] lg:h-[720px] overflow-hidden">
        ...
        <div className="relative z-10 h-full flex flex-col items-center justify-center text-center w-full max-w-[1440px] mx-auto px-4 md:px-[40px]">
          <div className="max-w-[800px] mt-16 md:mt-0">
            <h1 className="font-serif text-[24px] md:text-[62px] lg:text-[76px] leading-[1.1] text-white mb-4 md:mb-8 drop-shadow-lg uppercase tracking-tight">{dynamicContent?.heroTitle || "Rajasthan Romantique Kerala 14 Jours"} — {dynamicContent?.heroDuration ? (String(dynamicContent.heroDuration).toLowerCase().includes('jour') ? dynamicContent.heroDuration : `${dynamicContent.heroDuration} Jours`) : ""}</h1>
            <div className="w-12 md:w-16 h-px bg-[#c58b32] mb-3 md:mb-6 mx-auto" />
            <p className="text-[10px] md:text-[16px] leading-relaxed text-white/90 max-w-[800px] mx-auto drop-shadow-md">Un voyage imaginé pour les couples en quête d’émotions, de moments suspendus et d’expériences inoubliables.
              Entre palais royaux, dunes du Rajasthan, backwaters du Kerala et couchers de soleil romantiques, vivez une lune
              de miel élégante et profondément romantique en Inde.
            </p>
          </div>
        </div>
      </section>
...
export default LuneDeMielEscapadesRomantiques;
