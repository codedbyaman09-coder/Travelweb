...
const VoyagePhotoExpeditions = () => {
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([itinData, destData]) => {
        let itin = null;
        if (itinData.success) {
          itin = itinData.data.find(d => d.slug === 'voyage-photo-expeditions');
        }

        let dest = null;
        if (destData.success) {
          dest = destData.data.find(d => d.slug === 'voyage-photo-expeditions');
        }

        if (itin || dest) {
          let parsedContent = {};
          if ((itin && itin.page_content) || (dest && dest.page_content)) {
            try {
  ...
  const defaultHighlights = [
    " • Street photography authentique",
    " • Portraits & rencontres humaines",
    " • Rajasthan rural & villes royales",
    " • Dunes du désert au coucher du soleil",
    " • Taj Mahal & architecture moghole",
    " • Spiritualité unique de Varanasi",
    " • Voyage privé et immersif",
    " • Assistance francophone Indeora Voyages",
  ];
  const highlights = dynamicContent?.highlights?.length > 0 && dynamicContent.highlights[0].trim() !== "" ? dynamicContent.highlights : defaultHighlights;
    ...
    switch (activeTab) {
      case "ITINÉRAIRE":
        const defaultDays = [
          ...
          "day 5 — Bikaner : Vie Locale & Désert",
          "day 6 — Bikaner → Jodhpur",
          "day 7 — Jodhpur",
        ...
        const itineraryList = dynamicContent?.itinerary?.length > 0 && dynamicContent.itinerary[0].title?.trim() !== ""
          ? dynamicContent.itinerary.map(item => `day ${item.day} — ${item.title}`)
          : defaultDays;
          ...
          <div className="pt-12 pb-16 px-6 md:px-12 lg:px-16 animate-fadeIn">
            <div className="mb-12">
              <h2 className="text-[#000] font-serif text-[28px] md:text-[32px] font-bold mb-6 italic leading-tight">
                {dynamicContent?.heroTitle || "Rajasthan Authentique Varanasi"} — {dynamicContent?.heroDuration ? (String(dynamicContent.heroDuration).toLowerCase().includes('jour') ? dynamicContent.heroDuration : `${dynamicContent.heroDuration} Jours`) : "14 Jours"}
              </h2>
              <p className="text-[#000] font-['Montserrat'] leading-[1.8] text-[15px] max-w-3xl whitespace-pre-wrap">
                {dynamicContent?.heroDescription || "Un voyage pensé pour les amoureux de photographie, de rencontres humaines et d’ambiances authentiques. Des ruelles colorées du Rajasthan aux scènes spirituelles de Varanasi, cet itinéraire vous emmène à travers une Inde intense, esthétique et profondément vivante."}
              </p>
            </div>
      ...
      case "EN DÉTAIL":
        const defaultItinerary = [
          {
            day: "01", title: "Arrivée à Delhi", desc: (
              <div className="space-y-2">
                ...
                <p>• Transfert privé vers votre hôtel.</p>
                <p>• Première immersion dans New Delhi.</p>
                <p>• Balade dans Lodhi Garden.</p>
                <p>• Découverte de Connaught Place.</p>
                <p>• Premières scènes de street photography.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Une première immersion entre modernité et agitation indienne.
                </p>
              </div>
            )
          },
          {
            day: "02", title: "Old Delhi & Street Photography", desc: (
              <div className="space-y-2">
                <p>• Lever du soleil à Jama Masjid. </p>
                <p>• Balade en rickshaw à Chandni Chowk.</p>
                <p>• Marché aux épices de Khari Baoli.</p>
                <p>• Portraits de marchands et artisans.</p>
                <p>• Scènes de vie dans les ruelles anciennes.</p>
                <p>• Photographie urbaine et ambiance locale.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Couleurs, mouvements et regards uniques.</p>
              </div>
            )
          },
          {
            day: "03", title: "Delhi → Mandawa", desc: (
              <div className="space-y-2">
                <p>• Route vers la région du Shekhawati.</p>
                <p>• Arrêts photo dans les villages.</p>
                <p>• Découverte des havelis peintes.</p>
                <p>• Balade dans les ruelles historiques.</p>
                <p>• Rencontres avec les habitants.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Architecture ancienne et Rajasthan authentique.</p>
              </div>
            )
          },
          {
            day: "04", title: "Mandawa → Bikaner", desc: (
              <div className="space-y-2">
                <p>• Traversée des paysages désertiques.</p>
                <p>• Villages Bishnoï et scènes rurales.</p>
                <p>• Femmes en saris colorés.</p>
                <p>• Découverte des marchés de Bikaner.</p>
                <p>• Coucher du soleil dans les dunes.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Lumières dorées et atmosphère désertique.</p>
              </div>
            )
          },
          {
            day: "05", title: "Bikaner : Vie Locale & Désert", desc: (
              <div className="space-y-2">
                <p>• Rencontre avec des familles locales.</p>
                <p>• Visite de villages reculés.</p>
                <p>• Artisans et scènes rurales.</p>
                <p>• Musiciens locaux dans les dunes.</p>
                <p>• Photographie au coucher du soleil.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Un Rajasthan vivant et profondément humain.</p>
              </div>
            )
          },
          {
            day: "06", title: "Bikaner → Jodhpur", desc: (
              <div className="space-y-2">
                <p>• Arrêts thé chai sur la route.</p>
                <p>• Petits temples ruraux.</p>
                <p>• Découverte des ruelles bleues.</p>
                <p>• Clock Tower Market.</p>
                <p>• Photographie depuis les rooftops.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Entre désert et ville royale.</p>
              </div>
            )
          },
          {
            day: "07", title: "Jodhpur", desc: (
              <div className="space-y-2">
                <p>• Lever du soleil face au fort Mehrangarh.</p>
                <p>• Photographie des ruelles bleues.</p>
                <p>• Portraits d’artisans.</p>
                <p>• Marchés traditionnels.</p>
                <p>• Vie locale et street photography.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Couleurs vibrantes et patrimoine rajput.</p>
              </div>
            )
          },
          {
            day: "08", title: "Jodhpur → Pushkar", desc: (
              <div className="space-y-2">
                <p>• Traversée des villages du Rajasthan.</p>
                <p>• Découverte des ghats sacrés.</p>
                <p>• Rencontres avec les sadhus.</p>
                <p>• Marchés et ambiance bohème.</p>
                <p>• Coucher du soleil autour du lac.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Spiritualité et douceur de vivre.</p>
              </div>
            )
          },
          {
            day: "09", title: "Pushkar → Jaipur", desc: (
              <div className="space-y-2">
                <p>• Découverte du Hawa Mahal.</p>
                <p>• City Palace.</p>
                <p>• Marchés colorés.</p>
                <p>• Bijoux et textiles traditionnels.</p>
                <p>• Photographie nocturne dans les bazars.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Ville royale et énergie colorée.</p>
              </div>
            )
          },
          {
            day: "10", title: "Jaipur", desc: (
              <div className="space-y-2">
                <p>• Lever du soleil au Fort d’Amber.</p>
                <p>• Découverte du Jal Mahal.</p>
                <p>• Ateliers artisanaux.</p>
                <p>• Impression textile traditionnelle.</p>
                <p>• Street photography et scènes locales.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Traditions, savoir-faire et architecture.</p>
              </div>
            )
          },
          {
            day: "11", title: "Jaipur → Agra", desc: (
              <div className="space-y-2">
                <p>• Arrêt au Chand Baori d’Abhaneri.</p>
                <p>• Traversée de villages traditionnels.</p>
                <p>• Découverte du Taj Mahal au coucher du soleil.</p>
                <p>• Photographie architecturale moghole.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Élégance et lumière intemporelle.</p>
              </div>
            )
          },
          {
            day: "12", title: "Agra → Varanasi", desc: (
              <div className="space-y-2">
                <p>• Vol intérieur vers Varanasi.</p>
                <p>• Découverte des ghats.</p>
                <p>• Cérémonie Aarti sur le Gange.</p>
                <p>• Photographie spirituelle et scènes nocturnes.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Mystique et profondément spirituelle.</p>
              </div>
            )
          },
          {
            day: "13", title: "Varanasi", desc: (
              <div className="space-y-2">
                <p>• Balade en bateau au lever du soleil.</p>
                <p>• Vie quotidienne sur les ghats.</p>
                <p>• Portraits de sadhus.</p>
                <p>• Marchés anciens.</p>
                <p>• Temples cachés et scènes locales.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Une Inde spirituelle et émouvante.</p>
              </div>
            )
          },
          {
            day: "14", title: "Départ", desc: (
              <div className="space-y-2">
                ...
                <p>• Temps libre selon l’horaire du départ.</p>
                <p>• Assistance jusqu’à l’aéroport.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Fin de votre voyage photo en Inde.</p>
              </div>
            )
          },
        ];
        ...
        const itinerary = formattedDynamicItinerary && formattedDynamicItinerary.length > 0 && formattedDynamicItinerary[0].title?.trim() !== "" ? formattedDynamicItinerary : defaultItinerary;

        const activeItinerary = dynamicContent?.itinerary?.length > 0 && typeof dynamicContent.itinerary[0]?.title === 'string' && dynamicContent.itinerary[0].title.trim() !== "" ? dynamicContent.itinerary : itinerary;
        const selectedDay = activeItinerary[openDay] || activeItinerary[0] || itinerary[0];
          ...
          <div className="pt-12 pb-16 px-6 md:px-12 lg:px-16 animate-fadeIn">
            ...
            <div className="bg-[#fcfcfc] border border-gray-100 p-8 mb-10">
              <div className="flex justify-between items-center mb-6">
                ...
                <span className="text-3xl font-serif font-bold text-[#102d45]">{dynamicContent?.price || "860€"}</span>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
              <div>
                <h4 className="font-bold text-[#102d45] mb-4 uppercase text-sm tracking-widest border-b border-[#b7772e] pb-2 inline-block">Le prix comprende</h4>
              ...
              <div>
                <h4 className="font-bold text-[#102d45] mb-4 uppercase text-sm tracking-widest border-b border-gray-200 pb-2 inline-block">Le prix ne comprende pas</h4>
                <ul className="space-y-2 text-sm text-gray-600 italic">
      ...
      <section className="relative h-[300px] md:h-[650px] lg:h-[720px] overflow-hidden">
        ...
        <div className="relative z-10 h-full flex flex-col items-center justify-center text-center w-full max-w-[1440px] mx-auto px-4 md:px-[40px] mt-8 md:mt-0">
          <div className="max-w-[800px]">
            <h1 className="font-serif text-[28px] md:text-[62px] lg:text-[76px] leading-[1.1] text-white mb-4 md:mb-8 drop-shadow-lg uppercase tracking-tight">
              {dynamicContent?.heroTitle || "Rajasthan Authentique Varanasi"} — {dynamicContent?.heroDuration ? (String(dynamicContent.heroDuration).toLowerCase().includes('jour') ? dynamicContent.heroDuration : `${dynamicContent.heroDuration} Jours`) : "14 Jours"}
            </h1>
            <div className="w-12 md:w-16 h-px bg-[#c58b32] mb-4 md:mb-6 mx-auto" />
            <p className="text-[11px] md:text-[16px] leading-relaxed text-white/90 max-w-[800px] mx-auto drop-shadow-md whitespace-pre-wrap">
              {dynamicContent?.heroDescription || "Un voyage pensé pour les amoureux de photographie, de rencontres humaines et d’ambiances authentiques. Des ruelles colorées du Rajasthan aux scènes spirituelles de Varanasi, cet itinéraire vous emmène à travers une Inde intense, esthétique et profondément vivante."}
            </p>
          </div>
        </div>
            ...
            <div className="hidden md:block bg-white border border-[#eadfce]/40 rounded-sm p-10 text-center shadow-[0_15px_45px_rgba(70,45,20,0.12)]">
              ...
              <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-6 mb-12">
                <div>
                  ...
                  <p className="text-[24px] font-bold text-[#102d45]">{dynamicContent?.price ? (String(dynamicContent.price).includes('/') ? dynamicContent.price : `${dynamicContent.price}/pers`) : "860.00€/pers"}</p>
                </div>
...
export default VoyagePhotoExpeditions;
