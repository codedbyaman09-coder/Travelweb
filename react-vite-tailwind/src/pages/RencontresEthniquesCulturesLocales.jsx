...
const RencontresEthniquesCulturesLocales = () => {
  ...
  const [dynamicContent, setDynamicContent] = useState(null);
    ...
    .then(([itinData, destData]) => {
      let itin = null;
      if (itinData.success) {
        itin = itinData.data.find(d => d.slug.replace(/-$/, '') === 'rencontres-ethniques-cultures-locales');
      }

      let dest = null;
      if (destData.success) {
        dest = destData.data.find(d => d.slug.replace(/-$/, '') === 'rencontres-ethniques-cultures-locales');
      }
      ...
      case "ITINÉRAIRE":
        return (
          <div className="pt-12 pb-16 px-6 md:px-12 lg:px-16 animate-fadeIn">
            <div className="mb-12">
              <h2 className="text-[#000] font-serif text-[28px] md:text-[32px] font-bold mb-6 italic leading-tight">{dynamicContent?.heroTitle || "Rajasthan, Gujarat Inde Tribale — 15 Jours"} — {dynamicContent?.heroDuration ? (String(dynamicContent.heroDuration).toLowerCase().includes('jour') ? dynamicContent.heroDuration : `${dynamicContent.heroDuration} Jours`) : ""}</h2>
              <p className="text-[#000] font-['Montserrat'] leading-[1.8] text-[15px] max-w-3xl">{dynamicContent?.heroDescription || "Un voyage profondément humain à la découverte des peuples, traditions et cultures locales de l’Inde. Des villages du Rajasthan aux communautés tribales du Gujarat, vivez des rencontres authentiques loin du tourisme classique et découvrez une Inde intime, chaleureuse et préservée."}</p>
            </div>
            <div className="space-y-4 mb-16">
              {(dynamicContent?.itinerary?.length > 0 && typeof dynamicContent.itinerary[0]?.title === 'string' && dynamicContent.itinerary[0].title.trim() !== "" ? dynamicContent.itinerary : [
                ...
                { day: "4", title: "Villages Bishnoï & Traditions Locales," },
                { day: "5", title: "Bikaner → Jodhpur," },
                { day: "6", title: "Villages autour de Jodhpur," },
                { day: "7", title: "Jodhpur → Udaipur •," },
                { day: "8", title: "Udaipur : Culture & Vie Locale," },
                { day: "9", title: "Udaipur → Ahmedabad ," },
                { day: "10", title: "Ahmedabad → Bhuj ," },
                { day: "11", title: "Villages tribaux du Kutch," },
                { day: "12", title: "Désert Blanc du Kutch ," },
                { day: "13", title: "Bhuj → Ahmedabad  ," },
                { day: "14", title: "Ahmedabad → Delhi," },
                { day: "15", title: "Départ" }
              ]).map((item, index) => (
                <div
                  key={index}
      ...
      <section className="relative w-full h-[320px] md:h-[650px] lg:h-[720px] overflow-hidden">
        ...
        <div className="relative z-10 h-full flex flex-col items-center justify-center text-center w-full max-w-[1440px] mx-auto px-4 md:px-[40px]">
          <div className="max-w-[800px] mt-16 md:mt-0">
            <h1 className="font-serif text-[24px] md:text-[62px] lg:text-[76px] leading-[1.1] text-white mb-4 md:mb-8 drop-shadow-lg uppercase tracking-tight">{dynamicContent?.heroTitle || "Rajasthan, Gujarat Inde Tribale 15 Jours"} — {dynamicContent?.heroDuration ? (String(dynamicContent.heroDuration).toLowerCase().includes('jour') ? dynamicContent.heroDuration : `${dynamicContent.heroDuration} Jours`) : ""}</h1>
            ...
            <p className="text-[10px] md:text-[16px] leading-relaxed text-white/90 max-w-[800px] mx-auto drop-shadow-md">
              Un voyage profondément humain à la découverte des peuples, traditions et cultures locales de l’Inde. Des villages du Rajasthan aux communautés tribales du Gujarat, vivez des rencontres authentiques loin du tourisme classique et découvrez une Inde intime, chaleureuse et préservée.
            </p>
          </div>
        </div>
      </section>
