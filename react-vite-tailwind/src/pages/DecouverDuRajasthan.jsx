...
const DecouverDuRajasthan = () => {
  ...
  useEffect(() => {
    ...
    .then(([itinData, destData]) => {
      let itin = null;
      if (itinData.success) {
        itin = itinData.data.find(d => d.slug.replace(/-$/, '') === 'decouver-du-rajasthan');
      }
      
      let dest = null;
      if (destData.success) {
        dest = destData.data.find(d => d.slug.replace(/-$/, '') === 'decouver-du-rajasthan');
      }
  ...
  const highlights = [
    "• Palais & forts du Rajasthan",
    "• Jaisalmer & désert du Thar",
    "• Dîner sous les étoiles dans les dunes",
    "• Ville bleue de Jodhpur",
    "• Udaipur & ses palais romantiques",
    "• Jaipur la ville royale",
    "• Bharatpur & safari ornithologique",
          ...
          <div className="pt-12 pb-16 px-6 md:px-12 lg:px-16 animate-fadeIn">
            <div className="mb-12">
              <h2 className="text-[#000] font-serif text-[28px] md:text-[32px] font-bold mb-6 italic leading-tight">
                {dynamicContent?.heroTitle || "Rajasthan Royal Taj Mahal"} — {dynamicContent?.heroDuration ? (String(dynamicContent.heroDuration).toLowerCase().includes('jour') ? dynamicContent.heroDuration : `${dynamicContent.heroDuration} Jours`) : "15 Jours"}
              </h2>
              <p className="text-[#000] font-['Montserrat'] leading-[1.8] text-[15px] max-w-3xl whitespace-pre-wrap">
                {dynamicContent?.heroDescription || "Un voyage d'exception au cœur du Rajasthan royal, des ruelles animées de Delhi aux magnifiques palais d'Udaipur et au majestueux Taj Mahal. Plongez dans l'histoire des Maharajas, traversez le désert du Thar et découvrez des paysages à couper le souffle."}
              </p>
            </div>
            <div className="space-y-4 mb-16">
              {(dynamicContent?.itinerary?.length > 0 && typeof dynamicContent.itinerary[0]?.title === 'string' && dynamicContent.itinerary[0].title.trim() !== "" ? dynamicContent.itinerary : [
                ...
                { day: "5", title: "Jaisalmer & Désert du Thar" },
                { day: "6", title: "Jaisalmer → Jodhpur" },
                { day: "7", title: "Jodhpur" },
                { day: "8", title: "Jodhpur → Ranakpur → Udaipur" },
                { day: "9", title: "Udaipur" },
                { day: "10", title: "Udaipur → Pushkar" },
                { day: "11", title: "Pushkar → Jaipur" },
                { day: "12", title: "Jaipur" },
                { day: "13", title: "Jaipur → Bharatpur" },
                { day: "14", title: "Bharatpur → Fatehpur Sikri → Agra" },
                { day: "15", title: "Agra → Delhi & Départ" }
              ]).map((item, index) => (
                <div
                  key={index}
        ...
        const itinerary = [
          {
            day: "01", title: "Arrivée à Delhi", desc: (
              <div className="space-y-2">
                ...
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Une entrée élégante dans le nord de l’Inde.</p>
              </div>
            )
          },
          {
            day: "02", title: "Delhi → Mandawa", desc: (
              <div className="space-y-2">
                ...
                <p>• Fresques anciennes.</p>
                <p>• Marchés locaux.</p>
                <p>• Vie rurale du Rajasthan.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Premiers instants dans le Rajasthan traditionnel.</p>
              </div>
            )
          },
          {
            day: "03", title: "Mandawa → Bikaner", desc: (
              <div className="space-y-2">
                ...
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Le Rajasthan authentique.</p>
              </div>
            )
          },
          {
            day: "04", title: "Bikaner → Jaisalmer", desc: (
              <div className="space-y-2">
                <p>• Route à travers le désert.</p>
                <p>• Fort doré.</p>
                <p>• Ruelles anciennes.</p>
                <p>• Havelis du désert.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Une ville mythique du désert.</p>
              </div>
            )
          },
          {
            day: "05", title: "Jaisalmer & Désert du Thar", desc: (
              <div className="space-y-2">
                <p>• Balade à dos de chameau.</p>
                <p>• Coucher du soleil.</p>
                <p>• Danse Kalbeliya.</p>
                <p>• Dîner sous les étoiles.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Une soirée magique dans les dunes.</p>
              </div>
            )
          },
          {
            day: "06", title: "Jaisalmer → Jodhpur", desc: (
              <div className="space-y-2">
                <p>• Fort Mehrangarh.</p>
                <p>• Clock Tower Market.</p>
                <p>• Rues bleues.</p>
                <p>• Marchés aux épices.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">La célèbre ville bleue du Rajasthan.</p>
              </div>
            )
          },
          {
            day: "07", title: "Jodhpur", desc: (
              <div className="space-y-2">
                ...
                <p>• Rooftops avec vue sur le fort.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Patrimoine royal et ambiance locale.</p>
              </div>
            )
          },
          {
            day: "08", title: "Jodhpur → Ranakpur → Udaipur", desc: (
              <div className="space-y-2">
                <p>• Temples jaïns de Ranakpur.</p>
                <p>• Villages des Aravalli.</p>
                <p>• Lac Pichola.</p>
                <p>• Palais illuminés.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Une ville romantique et raffinée.</p>
              </div>
            )
          },
          {
            day: "09", title: "Udaipur", desc: (
              <div className="space-y-2">
                <p>• City Palace.</p>
                <p>• Vieille ville.</p>
                <p>• Marchés artisanaux.</p>
                <p>• Balade en bateau sur le lac.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Élégance et douceur du Rajasthan.</p>
              </div>
            )
          },
          {
            day: "10", title: "Udaipur → Pushkar", desc: (
              <div className="space-y-2">
                <p>• Lac sacré.</p>
                <p>• Ghats.</p>
                <p>• Temples.</p>
                <p>• Marchés bohèmes.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Une ville spirituelle et paisible.</p>
              </div>
            )
          },
          {
            day: "11", title: "Pushkar → Jaipur", desc: (
              <div className="space-y-2">
                <p>• Hawa Mahal.</p>
                <p>• City Palace.</p>
                <p>• Bazars colorés.</p>
                <p>• Bijoux et textiles traditionnels.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">La capitale royale du Rajasthan.</p>
              </div>
            )
          },
          {
            day: "12", title: "Jaipur", desc: (
              <div className="space-y-2">
                <p>• Fort d’Amber.</p>
                <p>• Jal Mahal.</p>
                <p>• Observatoire Jantar Mantar.</p>
                <p>• Ateliers artisanaux.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Traditions et patrimoine royal.</p>
              </div>
            )
          },
          {
            day: "13", title: "Jaipur → Bharatpur", desc: (
              <div className="space-y-2">
                <p>• Parc national de Keoladeo.</p>
                <p>• Safari en rickshaw.</p>
                <p>• Observation des oiseaux migrateurs.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Une parenthèse nature au cœur du Rajasthan.</p>
              </div>
            )
          },
          {
            day: "14", title: "Bharatpur → Fatehpur Sikri → Agra", desc: (
              <div className="space-y-2">
                <p>• Visite de Fatehpur Sikri.</p>
                <p>• Architecture moghole.</p>
                <p>• Taj Mahal au coucher du soleil.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Le chef-d’œuvre romantique de l’Inde.
                </p>
              </div>
            )
          }, {
            day: "15", title: "Agra → Delhi & Départ", desc: (
              <div className="space-y-2">
                <p>• Retour vers Delhi.</p>
                <p>• Temps libre.</p>
                <p>• Transfert vers l’aéroport.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Fin de votre grand voyage royal.
                </p>
              </div>
            )
          },
            ...
            <div className="flex flex-col md:flex-row gap-12 md:gap-20">
              ...
              <div className="flex-1">
                <div key={openDay} className="animate-fadeIn">
                  ...
                  <div className="text-[#444] leading-[1.85] text-[16px] font-['Montserrat'] pl-4 border-l-2 border-[#b7772e]/20 max-w-2xl whitespace-pre-wrap">
                    {selectedDay.desc}
                  </div>
          ...
          <div className="pt-12 pb-16 px-6 md:px-12 lg:px-16 animate-fadeIn">
            ...
            <div className="bg-[#fcfcfc] border border-gray-100 p-8 mb-10">
              <div className="flex justify-between items-center mb-6">
                ...
                <span className="text-3xl font-serif font-bold text-[#102d45]">{dynamicContent?.price || "1345€"}</span>
              </div>
              ...
              <div>
                ...
                <ul className="space-y-2 text-sm text-gray-600 italic">
                  {(dynamicContent?.priceIncludes ? dynamicContent.priceIncludes.split('\n') : [
                      "• L'hébergement de 14 nuits en chambre double de catégorie charme",
      ...
      <section className="relative h-[300px] md:h-[650px] lg:h-[720px] overflow-hidden">
        ...
        <div className="relative z-10 h-full flex flex-col items-center justify-center text-center w-full max-w-[1440px] mx-auto px-4 md:px-[40px]">
          <div className="max-w-[800px] mt-8 md:mt-0">
            <h1 className="font-serif text-[24px] md:text-[62px] lg:text-[76px] leading-[1.1] text-white mb-2 md:mb-8 drop-shadow-lg uppercase tracking-tight">
              {dynamicContent?.heroTitle || "Rajasthan Royal Taj Mahal"} — {dynamicContent?.heroDuration ? (String(dynamicContent.heroDuration).toLowerCase().includes('jour') ? dynamicContent.heroDuration : `${dynamicContent.heroDuration} Jours`) : "15 Jours"}
            </h1>
            <div className="w-10 md:w-16 h-px bg-[#c58b32] mb-3 md:mb-6 mx-auto" />
            <p className="text-[10px] md:text-[16px] leading-relaxed text-white/90 max-w-[800px] mx-auto drop-shadow-md px-2 whitespace-pre-wrap">
              {dynamicContent?.heroDescription || "Des palais raffinés de Jaipur aux ruelles bleues de Jodhpur, des dunes dorées du désert aux merveilles mogholes d’Agra, ce voyage vous plonge au cœur du Rajasthan éternel."}
            </p>
          </div>
        </div>
      </section>
            ...
            <div className="hidden md:block bg-white border border-[#eadfce]/40 rounded-sm p-10 text-center shadow-[0_15px_45px_rgba(70,45,20,0.12)]">
              ...
              <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-6 mb-12">
                <div>
                  ...
                  <p className="text-[24px] font-bold text-[#102d45]">{dynamicContent?.price ? (String(dynamicContent.price).includes('/') ? dynamicContent.price : `${dynamicContent.price}/pers`) : "1345€/pers"}</p>
                </div>
                ...
                <div>
                  ...
                  <p className="text-[24px] font-bold text-[#102d45] whitespace-nowrap">{dynamicContent?.durationText || "15j / 14n"}</p>
                </div>
              </div>
...
export default DecouverDuRajasthan;
