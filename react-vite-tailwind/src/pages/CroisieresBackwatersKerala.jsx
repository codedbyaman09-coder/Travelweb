...
const CroisieresBackwatersKerala = () => {
  ...
  const [dynamicContent, setDynamicContent] = useState(null);
    ...
    .then(([itinData, destData]) => {
      let itin = null;
      if (itinData.success) {
        itin = itinData.data.find(d => d.slug.replace(/-$/, '') === 'croisieres-backwaters-kerala');
      }

      let dest = null;
      if (destData.success) {
        dest = destData.data.find(d => d.slug.replace(/-$/, '') === 'croisieres-backwaters-kerala');
      }
  ...
  const defaultHighlights = [
    ...
    " • Plantations de thé & épices",
    " • Ayurveda & bien-être",
    " • Villages authentiques du Kerala",
    " • Nature tropicale & plages paisibles",
    " • Cuisine locale du sud de l’Inde",
    ...
    switch (activeTab) {
      case "ITINÉRAIRE":
        return (
          <div className="pt-12 pb-16 px-6 md:px-12 lg:px-16 animate-fadeIn">
            <div className="mb-12">
              <h2 className="text-[#000] font-serif text-[28px] md:text-[32px] font-bold mb-6 italic leading-tight">{dynamicContent?.heroTitle || "Kerala Tropical Houseboat Experience — 13 Jours"} — {dynamicContent?.heroDuration ? (String(dynamicContent.heroDuration).toLowerCase().includes('jour') ? dynamicContent.heroDuration : `${dynamicContent.heroDuration} Jours`) : ""}</h2>
              <p className="text-[#000] font-['Montserrat'] leading-[1.8] text-[15px] max-w-3xl">{dynamicContent?.heroDescription || "Un voyage au rythme lent du Kerala, entre nature luxuriante, villages paisibles, Ayurveda et célèbres backwaters. Découvrez une Inde tropicale et apaisante à travers des croisières privées, des plantations de thé, des plages bordées de cocotiers et une immersion authentique dans la vie locale du sud de l’Inde."}</p>
            </div>
            <div className="space-y-4 mb-16">
              {(dynamicContent?.itinerary?.length > 0 && typeof dynamicContent.itinerary[0]?.title === 'string' && dynamicContent.itinerary[0].title.trim() !== "" ? dynamicContent.itinerary : [
                { day: "1", title: "Arrivée à Kochi ," },
                { day: "2", title: "Kochi : Culture & Vie Locale," },
                { day: "3", title: "Kochi → Munnar," },
                { day: "4", title: "Munnar : Nature & Plantations," },
                { day: "5", title: "Munnar → Thekkady," },
                { day: "6", title: "Thekkady : Nature & Épices," },
                { day: "7", title: "Thekkady → Alleppey," },
                { day: "8", title: "Croisière dans les Backwaters," },
                { day: "9", title: "Alleppey → Kumarakom," },
                { day: "10", title: "Kumarakom → Marari Beach," },
                { day: "11", title: "Marari Beach : Journée Libre," },
                { day: "12", title: "Marari → Kochi," },
                { day: "13", title: "Départ," }
              ]).map((item, index) => (
                <div
                  key={index}
      ...
      case "EN DÉTAIL":
        const defaultItinerary = [
          {
            day: "01", title: "Arrivée à Kochi", desc: (
              <div className="space-y-2">
                <p>• Accueil personnalisé à l’aéroport.</p>
                <p>• Découverte de Fort Kochi.</p>
                <p>• Filets de pêche chinois.</p>
                <p>• Rues historiques et cafés.</p>
                <p>• Spectacle traditionnel de Kathakali.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Premiers instants dans l’ambiance tropicale du Kerala.</p>
              </div>
            )
          },
          {
            day: "02", title: "Kochi : Culture & Vie Locale", desc: (
              <div className="space-y-2">
                <p>• Marchés aux épices.</p>
                <p>• Églises portugaises.</p>
                <p>• Synagogue de Mattancherry.</p>
                <p>• Cuisine locale du Kerala..</p>
                <p>• Balade au bord de la mer.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Une ville entre influences coloniales et traditions du sud de l’Inde.</p>
              </div>
            )
          },
          {
            day: "03", title: "Kochi → Munnar", desc: (
              <div className="space-y-2">
                <p>• Route vers les montagnes du Kerala.</p>
                <p>• Cascades tropicales.</p>
                <p>• Plantations de thé.</p>
                <p>• Paysages verdoyants.</p>
                <p>• Première découverte de Munnar.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Nature luxuriante et fraîcheur des montagnes.</p>
              </div>
            )
          },
          {
            day: "04", title: "Munnar : Nature & Plantations", desc: (
              <div className="space-y-2">
          ...
          {
            day: "05", title: "Munnar → Thekkady", desc: (
              <div className="space-y-2">
                <p>• Route vers Thekkady.</p>
                <p>• Plantations d’épices.</p>
                <p>• Villages du Kerala.</p>
                <p>• Spectacle d’arts martiaux Kalaripayattu.</p>
                <p>• Massage ayurvédique traditionnel.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Entre jungle tropicale et traditions locales.</p>
              </div>
            )
          },
          {
            day: "06", title: "Thekkady : Nature & Épices", desc: (
              <div className="space-y-2">
          ...
          {
            day: "07", title: "Thekkady → Alleppey", desc: (
              <div className="space-y-2">
                <p>• Arrivée dans la région des backwaters.</p>
                <p>• Installation dans un houseboat privé.</p>
                <p>• Début de la croisière.</p>
          ...
          {
            day: "08", title: "Croisière dans les Backwaters", desc: (
              <div className="space-y-2">
                <p>• Navigation paisible.</p>
                <p>• Découverte de la vie locale.</p>
                <p>• Pêche traditionnelle.</p>
                <p>• Écoles flottantes.</p>
                <p>• Cuisine locale à bord.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]"></p>Le Kerala authentique au fil de l’eau.
              </div>
            )
          },
          {
            day: "09", title: "Alleppey → Kumarakom", desc: (
              <div className="space-y-2">
                <p> • Balade en canoë traditionnel.</p>
                <p>• Observation des oiseaux.</p>
                <p>• Villages cachés.</p>
                <p>• Temps libre au bord du lac Vembanad.</p>
                <p>• Spa ou Ayurveda.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Nature paisible et sérénité tropicale.</p>
              </div>
            )
          },
          {
            day: "10", title: "Kumarakom → Marari Beach", desc: (
              <div className="space-y-2">
                <p>• Départ vers les plages de Marari.</p>
                <p>• Temps libre à la plage.</p>
                <p>• Massage ayurvédique.</p>
                <p>• Balade au coucher du soleil.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Repos et douceur tropicale.</p>
              </div>
            )
          },
          {
            day: "11", title: "Marari Beach : Journée Libre", desc: (
              <div className="space-y-2">
                <p>• Yoga face à la mer.</p>
                <p>• Temps libre à la plage.</p>
                <p>• Balade à vélo dans les villages.</p>
                <p>• Ayurveda et relaxation.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Une journée dédiée au bien-être.</p>
              </div>
            )
          },
          {
            day: "12", title: "Marari → Kochi", desc: (
              <div className="space-y-2">
                <p>• Retour vers Kochi.</p>
                <p>• Shopping artisanal.</p>
                <p>• Café en bord de mer.</p>
                <p>• Derniers moments au Kerala.</p>
                <p>• Dîner d’adieu traditionnel.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Une dernière immersion dans l’ambiance tropicale.</p>
              </div>
            )
          },
          {
            day: "13", title: "Départ", desc: (
              <div className="space-y-2">
                ...
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Fin de votre voyage tropical au Kerala.</p>
              </div>
            )
          },
      ...
      <section className="relative h-[300px] md:h-[650px] lg:h-[720px] overflow-hidden">
        ...
        <div className="relative z-10 h-full flex flex-col items-center justify-center text-center w-full max-w-[1440px] mx-auto px-4 md:px-[40px] mt-8 md:mt-0">
          <div className="max-w-[800px]">
            <h1 className="font-serif text-[28px] md:text-[62px] lg:text-[76px] leading-[1.1] text-white mb-4 md:mb-8 drop-shadow-lg uppercase tracking-tight">{dynamicContent?.heroTitle || "Kerala Tropical Houseboat 13 Jours"} — {dynamicContent?.heroDuration ? (String(dynamicContent.heroDuration).toLowerCase().includes('jour') ? dynamicContent.heroDuration : `${dynamicContent.heroDuration} Jours`) : ""}</h1>
            ...
            {otherTrips.map((trip, index) => (
              <div key={index} className="group relative h-[250px] md:h-[450px] overflow-hidden rounded-sm shadow-xl cursor-pointer">
                ...
                <div className="absolute inset-x-0 bottom-0 p-3 md:p-6 text-left flex flex-col items-start gap-0.5 md:gap-1">
                  ...
                  <a href="#" className="text-white text-[8px] md:text-[12px] font-bold tracking-widest uppercase border-b border-white/40 hover:border-white transition-all">&gt; DÉCOUVRIR</a>
                </div>
              </div>
            ))}
          </div>
...
export default CroisieresBackwatersKerala;
