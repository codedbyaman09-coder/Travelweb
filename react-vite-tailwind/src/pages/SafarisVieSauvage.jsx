...
const SafarisVieSauvage = () => {
  ...
  useEffect(() => {
    ...
    .then(([itinData, destData]) => {
      let itin = null;
      if (itinData.success) {
        itin = itinData.data.find(d => d.slug.replace(/-$/, '') === 'safaris-vie-sauvage');
      }

      let dest = null;
      if (destData.success) {
        dest = destData.data.find(d => d.slug.replace(/-$/, '') === 'safaris-vie-sauvage');
      }
  ...
  const defaultHighlights = [
    " • Safaris dans les plus beaux parcs d’Inde",
    " • Observation des tigres du Bengale",
    " • Ranthambore, Bandhavgarh & Kanha",
    " • Nature sauvage & photographie animalière",
    " • Lodges au cœur de la jungle",
    " • Taj Mahal & Rajasthan royal",
    " • Expériences authentiques en pleine nature",
          ...
          <div className="pt-12 pb-16 px-6 md:px-12 lg:px-16 animate-fadeIn">
            <div className="mb-12">
              <h2 className="text-[#000] font-serif text-[28px] md:text-[32px] font-bold mb-6 italic leading-tight">
                {dynamicContent?.heroTitle || "Inde Sauvage Safaris — 14 Jours"} — {dynamicContent?.heroDuration ? (String(dynamicContent.heroDuration).toLowerCase().includes('jour') ? dynamicContent.heroDuration : `${dynamicContent.heroDuration} Jours`) : ""}
              </h2>
              <p className="text-[#000] font-['Montserrat'] leading-[1.8] text-[15px] max-w-3xl">{dynamicContent?.heroDescription || "Un voyage au cœur de l’Inde sauvage, pensé pour les amoureux de nature, d’animaux et d’aventure. Des tigres du Bengale aux léopards, des forêts du Rajasthan aux paysages du Madhya Pradesh, partez pour une immersion unique dans les plus beaux parcs nationaux d’Inde."}</p>
            </div>
            <div className="space-y-4 mb-16">
              {(dynamicContent?.itinerary?.length > 0 && typeof dynamicContent.itinerary[0]?.title === 'string' && dynamicContent.itinerary[0].title.trim() !== "" ? dynamicContent.itinerary : [
                ...
                "day 3 — Ranthambore : Safari Tigres du Bengale",
                "day 4 — Ranthambore : Nature & Vie Sauvage",
                "day 5 — Ranthambore → Jaipur",
                "day 6 — Jaipur → Agra",
                "day 7 — Agra → Delhi → Jabalpur",
                "day 8 — Bandhavgarh : Safari Nature",
                "day 9 — Bandhavgarh : Jungle Indienne",
                "day 10 — Bandhavgarh → Kanha",
                "day 11 — Kanha National Park",
                "day 12 — Kanha : Nature & Photographie",
                "day 13 — Kanha → Jabalpur → Delhi",
                "day 14 — Départ"
              ]).map((item, index) => (
                <div
                  key={index}
        ...
        const defaultItinerary = [
          {
            day: "01", title: "Arrivée à Delhi", desc: (
              <div className="space-y-2">
                ...
                <p>• Balade dans les jardins et marchés locaux.</p>
                <p>• Immersion dans l’ambiance indienne.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Une entrée douce avant l’aventure sauvage.</p>
              </div>
            )
          },
          {
            day: "02", title: "Delhi → Ranthambore", desc: (
              <div className="space-y-2">
                <p>• Départ vers le parc national de Ranthambore.</p>
                <p>• Installation dans un lodge nature.</p>
                <p>• Première ambiance safari.</p>
                <p>• Observation des paysages sauvages.</p>
                <p>• Coucher du soleil dans la jungle.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Premiers instants au cœur de la nature indienne.</p>
              </div>
            )
          },
          {
            day: "03", title: "Ranthambore : Safari Tigres du Bengale", desc: (
              <div className="space-y-2">
                <p>• Safari matinal dans le parc.</p>
                <p>• Possibilité d’observer tigres, léopards et crocodiles.</p>
                <p>• Photographie animalière.</p>
                <p>• Deuxième safari au coucher du soleil.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">L’excitation unique des safaris indiens.</p>
              </div>
            )
          },
          {
            day: "04", title: "Ranthambore : Nature & Vie Sauvage", desc: (
              <div className="space-y-2">
                <p>• Exploration des différentes zones du parc.</p>
                <p>• Observation des oiseaux exotiques.</p>
                <p>• Paysages de jungle.</p>
                <p>• Temps libre au lodge.</p>
                <p>• Lever du soleil dans la nature sauvage.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Une immersion totale dans la jungle.</p>
              </div>
            )
          },
          {
            day: "05", title: "Ranthambore → Jaipur", desc: (
              <div className="space-y-2">
                ...
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Entre aventure sauvage et patrimoine royal.</p>
              </div>
            )
          },
          {
            day: "06", title: "Jaipur → Agra", desc: (
              <div className="space-y-2">
                <p>• Arrêt à Chand Baori.</p>
                <p>• Traversée de villages traditionnels.</p>
                <p>• Découverte du Taj Mahal au coucher du soleil.</p>
                <p>• Photographie architecturale.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Émotion et élégance moghole.</p>
              </div>
            )
          },
          {
            day: "07", title: "Agra → Delhi → Jabalpur", desc: (
              <div className="space-y-2">
                <p>• Vol ou train vers le Madhya Pradesh.</p>
                <p>• Installation proche des parcs nationaux.</p>
                <p>• Temps libre et repos.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Transition vers les grandes jungles du centre de l’Inde.</p>
              </div>
            )
          },
          {
            day: "08", title: "Bandhavgarh : Safari Nature", desc: (
              <div className="space-y-2">
                <p>• Safari matinal.</p>
                <p>• Observation des tigres et ours paresseux.</p>
                <p>• Forêts tropicales.</p>
                <p>• Deuxième safari dans une autre zone.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Une Inde sauvage et préservée.</p>
              </div>
            )
          },
          {
            day: "09", title: "Bandhavgarh : Jungle Indienne", desc: (
              <div className="space-y-2">
                <p>• Safari photo.</p>
                <p>• Observation animalière.</p>
                <p>• Villages tribaux proches.</p>
                <p>• Temps libre dans la nature.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Une immersion authentique dans les jungles indiennes.</p>
              </div>
            )
          },
          {
            day: "10", title: "Bandhavgarh → Kanha", desc: (
              <div className="space-y-2">
                <p>• Route vers Kanha.</p>
                <p>• Traversée des paysages du Madhya Pradesh.</p>
                <p>• Installation dans un lodge nature.</p>
                <p>• Temps libre et détente.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Les paysages mythiques du Livre de la Jungle.</p>
              </div>
            )
          },
          {
            day: "11", title: "Kanha National Park", desc: (
              <div className="space-y-2">
                <p>• Safaris matin et après-midi.</p>
                <p>• Observation des tigres.</p>
                <p>• Découverte des forêts majestueuses.</p>
                <p>• Cerfs barasingha et gaurs indiens.</p>
                <p>• Photographie nature.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Une des plus belles réserves naturelles d’Inde.</p>
              </div>
            )
          },
          {
            day: "12", title: "Kanha : Nature & Photographie", desc: (
              <div className="space-y-2">
                <p>• Dernier safari photo.</p>
                <p>• Lever du soleil dans la jungle.</p>
                <p>• Observation silencieuse de la faune.</p>
                <p>• Temps de détente dans la nature.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Des souvenirs sauvages inoubliables.</p>
              </div>
            )
          },
          {
            day: "13", title: "Kanha → Jabalpur → Delhi", desc: (
              <div className="space-y-2">
                <p>• Retour vers Delhi.</p>
                <p>• Temps libre.</p>
                <p>• Dîner d’adieu.</p>
                <p>• Derniers souvenirs du voyage.</p>
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Fin progressive de l’aventure.</p>
              </div>
            )
          },
          {
            day: "14", title: "Départ", desc: (
              <div className="space-y-2">
                ...
                <p className="mt-6 font-medium italic text-[#102d45] text-[16px]">Fin de votre aventure safari en Inde.</p>
              </div>
            )
          },
        ];
        ...
        <div className="relative z-10 h-full flex flex-col items-center justify-center text-center w-full max-w-[1440px] mx-auto px-4 md:px-[40px]">
          <div className="max-w-[800px] mt-16 md:mt-0">
            <h1 className="font-serif text-[24px] md:text-[62px] lg:text-[76px] leading-[1.1] text-white mb-4 md:mb-8 drop-shadow-lg uppercase tracking-tight">
              {dynamicContent?.heroTitle || "Inde Sauvage Tigres du Bengale 14 Jours"} — {dynamicContent?.heroDuration ? (String(dynamicContent.heroDuration).toLowerCase().includes('jour') ? dynamicContent.heroDuration : `${dynamicContent.heroDuration} Jours`) : ""}
            </h1>
            <div className="w-12 md:w-16 h-px bg-[#c58b32] mb-3 md:mb-6 mx-auto" />
            <p className="text-[10px] md:text-[16px] leading-relaxed text-white/90 max-w-[800px] mx-auto drop-shadow-md">Un voyage au cœur de l’Inde sauvage, pensé pour les amoureux de nature, d’animaux et d’aventure. Des tigres
              du Bengale aux léopards, des forêts du Rajasthan aux paysages du Madhya Pradesh, partez pour une immersion
              unique dans les plus beaux parcs nationaux d’Inde.
            </p>
          </div>
        </div>
      </section>
...
export default SafarisVieSauvage;
